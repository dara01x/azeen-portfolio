  ...
  return (
    <div>
      ...
      <Card className="p-1.5">
        ...
        {filtered.length === 0 ? (
          <div className="p-6"><EmptyState title="No users found" description="Try adjusting filters or add a new user." action={<Button asChild><Link to="/users/new"><Plus className="mr-2 h-4 w-4" />Add User</Link></Button>} /></div>
        ) : (
          <Table>
            ...
            <TableBody>
              {filtered.map((u) => (
                <TableRow key={u.id} className="group">
                  ...
                  <TableCell><Button variant="ghost" size="sm" className="h-7 text-xs opacity-0 group-hover:opacity-100 transition-opacity" asChild><Link to={`/users/${u.id}/edit`}>Edit</Link></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
