  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        {stats.map((s) => (
          <Link to={s.href} key={s.label} className="group">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="ghost" size="sm" asChild className="text-muted-foreground">
                <Link to="/properties">View all <ArrowRight className="ml-1 h-3 w-3" /></Link>
              </Button>
            </CardHeader>
            <CardContent className="p-0">
              <div className="divide-y">
                {mockProperties.slice(0, 5).map((p) => (
                  <Link to={`/properties/${p.id}`} key={p.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-muted/50 transition-colors">
            ...
            <CardContent className="p-0">
              <div className="divide-y">
                {mockProjects.filter(p => p.status === "active").map((p) => (
                  <Link to={`/projects/${p.id}/edit`} key={p.id} className="flex items-center justify-between px-6 py-3 hover:bg-muted/50 transition-colors">
                    <div>
          ...
          <Card>
            ...
            <CardContent className="grid gap-2">
              <Button variant="outline" className="justify-start h-9" asChild>
                <Link to="/properties/new"><Building2 className="mr-2 h-3.5 w-3.5" />New Property</Link>
              </Button>
              <Button variant="outline" className="justify-start h-9" asChild>
                <Link to="/clients/new"><Users className="mr-2 h-3.5 w-3.5" />New Client</Link>
              </Button>
              <Button variant="outline" className="justify-start h-9" asChild>
                <Link to="/projects/new"><FolderKanban className="mr-2 h-3.5 w-3.5" />New Project</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
