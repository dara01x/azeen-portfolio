  ...
  return (
    <div>
      ...
      <Card className="p-1.5">
        ...
        {filtered.length === 0 ? (
          <div className="p-6"><EmptyState title="No clients found" description="Try adjusting filters or add a new client." action={<Button asChild><Link to="/clients/new"><Plus className="mr-2 h-4 w-4" />Add Client</Link></Button>} /></div>
        ) : (
          <Table>
            ...
            <TableBody>
              {filtered.map((c) => (
                <TableRow key={c.id} className="group">
                  ...
                  <TableCell><Button variant="ghost" size="sm" className="h-7 text-xs opacity-0 group-hover:opacity-100 transition-opacity" asChild><Link to={`/clients/${c.id}/edit`}>Edit</Link></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
