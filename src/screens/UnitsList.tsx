...
const UnitsList = () => {
  ...
  return (
    <div>
      <PageHeader title="Units" description="Manage project units" actions={<Button asChild><Link href="/units/new"><Plus className="mr-2 h-4 w-4" />Add Unit</Link></Button>} />
      <Card className="p-1.5">
        ...
        {filtered.length === 0 ? (
          <div className="p-6"><EmptyState title="No units found" description="Try adjusting filters or add a new unit." action={<Button asChild><Link href="/units/new"><Plus className="mr-2 h-4 w-4" />Add Unit</Link></Button>} /></div>
        ) : (
          <Table>
            ...
            <TableBody>
              {filtered.map((u) => (
                <TableRow key={u.id} className="group">
                  ...
                  <TableCell><Button variant="ghost" size="sm" className="h-7 text-xs opacity-0 group-hover:opacity-100 transition-opacity" asChild><Link href={`/units/${u.id}/edit`}>Edit</Link></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
