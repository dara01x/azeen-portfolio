...
export function ImageUpload({
  ...
  const handleFilesSelected = (files: FileList | null) => {
    ...
    if (acceptedFiles.length === 0) {
      setUploadError("No valid images were selected. Use image files up to 5MB each.");
      return;
    }
    ...
    if (oversizedFiles.length > 0 || droppedByCount > 0 || ignoredNonImages > 0) {
      ...
      if (oversizedFiles.length > 0) {
        messages.push(`${oversizedFiles.length} file(s) were larger than 5MB`);
      }
      if (droppedByCount > 0) {
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-muted-foreground">Max {MAX_IMAGE_COUNT} images, 5MB each.</p>
